...
function formatList() {
  ...
  arr.forEach((item) => {
      ...
      for (i in code) {if(i==0){codeon+=code[i]+" ";}else{codeon+="\n"+code[i]+" "}}res.push(codeon+"</pre>")
          ...
          try { regexMatch=item.match(/#link (.+?)($| )/g);i=0;while (i<regexMatch.length) {regexHolder=regexMatch[i].match(/#link (.+?)($| )/);link=regexHolder[1];finalElem=`<a href="">`+link+`</a>`;item=item.replace("#link "+link,finalElem);i++;} } catch {}
          ...
          try { regexMatch=item.match(/#anchorlink .+? (.+?)#endanchorlink/g);i=0;while (i<regexMatch.length) {regexHolder=regexMatch[i].match(/#anchorlink (.+?) (.+?)#endanchorlink/);link=regexHolder[2];finalElem=`<a href="">`+link+`</a>`;item=item.replace("#anchorlink "+regexHolder[1]+" "+link+"#endanchorlink",finalElem);i++;}} catch {}
